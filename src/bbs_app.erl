...
start(_StartType, _StartArgs) ->
  ...
  RootGrain = erleans:get_grain(bbs_statefull_agent_grain, <<"root">>),
...
get_tree(NodeId, Acc, Depth) ->
  case get_node(NodeId) of
    [] ->
      ...
      Acc;
    ChildList ->
      lists:map(fun ({_Node, Name, ChildNode}) -> #{name => Name, depth => Depth, node => NodeId, children => 
