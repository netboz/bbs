...
-spec(init(Args :: term()) ->
  {ok, State :: #bbs_lobby_state{}} | {ok, State :: #bbs_lobby_state{}, timeout() | hibernate} |
...
-spec(handle_call(Request :: term(), From :: {pid(), Tag :: term()},
  ...
  {reply, Reply :: term(), NewState :: #bbs_lobby_state{}, timeout() | hibernate} |
...
-spec(terminate(Reason :: (normal | shutdown | {shutdown, term()} | term()),
    State :: #bbs_lobby_state{}) -> term()).
terminate(_Reason, _State = #bbs_lobby_state{}) ->
  ok.

%% @private
%% @doc Convert process state when code is changed
-spec(code_change(OldVsn :: term() | {down, term()}, State :: #bbs_lobby_state{},
    Extra :: term()) ->
...
test() ->
  ...
  {ok,#{entity_id := EntId}, _Headers} =
    pod_service_entity_service_client:create_entity(
      #{name => <<"entity1">>, activate=>true, initial_position => #{x => 0, y => 0, z => 0}}),
  lager:info("Entity Id : ~p",[EntId]),
  pod_service_shape_component_service_client:add_shape_component(
    #{entity_id => EntId, type => primitive,
      parameters => {primitive_parameters,
        #{primitive_type => 'sphere',
          primitive_parameters => {primitive_sphere_parameters,#{radius => 5}}}}}),
  ...
  {ok,#{entity_id := EntId2}, _Headers} = pod_service_entity_service_client:create_entity(
    #{name => <<"entity2">>, activate=>true, initial_position => #{x => 10, y => 0, z => 0}}),
  lager:info("Entity 2 Id : ~p",[EntId2]),
  pod_service_shape_component_service_client:add_shape_component(
    #{entity_id => EntId2, type => primitive,
      parameters => {primitive_parameters,
        #{primitive_type => 'sphere',
          primitive_parameters => {primitive_sphere_parameters,#{radius => 4}}}}}),
...
test_basic_scene() ->
  ...
  {ok,PlayerId, _Headers} = e_gem_entity_service_client:create_entity(#{name => <<"Bob">>, activate=>false,
    initial_position => #{x => 1, y => 1, z => 1}}),

  PlayerCameraResult = egem_camera_service_client:add_camera_component(#{entity_id => PlayerId}),

  egem_entity_service_client:activate_entity(PlayerId),

  lager:info("Camera creation result :~p",[PlayerCameraResult]),

  {ok,#{entity_id := BulleId}, _Headers}  = egem_entity_service_client:create_entity(#{name => <<"Bulle">>, activate=>true,
    initial_position => #{x => 0, y => 0, z => 0}}),

  ResultBulleCreate = egem_shape_service_client:add_shape_component(
      #{entity_id => BulleId, type => mesh,
      parameters => {mesh_parameters,
        #{mesh_source => 'asset_processor', mesh_location => "Gem/Assets/bulleglasstext1.fbx"
          }}}),
  lager:info("Bubble creation result :~p",[ResultBulleCreate]),

  {ok, #{entity_id := LightId}, _Headers} = egem_entity_service_client:create_entity(#{name => <<"Light">>, activate=>false,
    initial_position => #{x => 0, y => 10, z => 0}}),

  lager:info("LightId :~p",[LightId]),

  LightCreationResult = egem_light_service_client:add_light_component(#{entity_id => LightId, is_visible => true}),
