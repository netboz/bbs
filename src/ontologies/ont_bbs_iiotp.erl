...
-define(PROLOG_PREDS, [{file, "./ont_bbs_iiotp.pl"}]).

%% API
-export([external_predicates/0, prolog_predicates/0, namespace/0]).
-export([new_acc/3, new_public_acc/3, send_msg_iiotp/3, public_acc/3,
         printable_all_acc/3]).
...
printable_all_acc({_Atom, Accs}, Next0, #est{} = St) ->
    ...
    ConvRes =
        lists:map(fun ({iiotp, Key, Ag}) when is_list(Key) ->
                          {iiotp, Key, Ag};
                      ({iiotp, Key, Ag}) when is_atom(Key) ->
                          {iiotp, atom_to_binary(Key, utf8), Ag}
                  end,
                  [Iiotp || [{_, _, Iiotp}, _, _] <- Res]),
...
send_msg_iiotp({_Atom, SenderAid, DestAid, Acc, Ontology, Msg}, Next0, St) ->
    ...
    gproc:send({p, l, Acc},
               {<<"bbs:iiotp">>, {iiotp_message, Acc, SenderAid, DestAid, Ontology, ConvPred}}),
    R = io_lib:format("~p", [ConvPred]),
    F = lists:flatten(R),
    lager:info("Convpred :~p", [ConvPred]),

    lager:info("Logging :~p", [F]),
    plantuml_logger:log_plant_message(
        maps:get(name, SenderAid), maps:get(name, DestAid), "ont_bbs_iiotp", ConvPred),
